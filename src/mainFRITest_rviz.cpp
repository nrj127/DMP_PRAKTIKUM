...
int main(int argc, char *argv[])
{
    ...
    ros::init(argc,argv, "main_drawing_rviz");

	if(!ros::master::check()){
		cout << "Please run roscore before starting this node" << endl;	
		return(0);
	}
    ...
    // WSG50 Gripper
    jointStateMsg.name.push_back("wsg50_gripper_base_joint_left");
    jointStateMsg.name.push_back("wsg50_gripper_base_joint_right");
    for (i = 0; i < LBR_MNJ+2; i++){
        jointStateMsg.position.push_back(0.0);
    }

    float jointAngles[LBR_MNJ];
        ...
        case 'E':
            ...
            if(startCartImpedanceCtrl(FRI, currentCartPose)==0){
                ...
                while ((FRI->IsMachineOK()) && it_<dim_){
                    ...
                    FRI->GetMeasuredJointPositions(jointAngles);
				    for(jIt_=0; jIt_<LBR_MNJ; ++jIt_){
				        jointStateMsg.position[jIt_] = jointAngles[jIt_];
				    }
				    jointStateMsg.header.stamp = ros::Time::now();
				    jointStateMsg.header.seq   = seq++;
